...
open import Relation.Nullary using (Dec; yes; ¬_) renaming (no to not)
...
module Semantics (nameCN namePN nameVI nameVT nameAdj : Set)
  ...
  mutual
    ...
    _≟_ : (x y : sCN) → Dec (x ≡ y)
    use-cn x ≟ use-cn y = map′ (cong use-cn) f (eqCN x y)
      where
      f : {x y : nameCN} → (sCN.use-cn x ≡ use-cn y) → x ≡ y
      f refl = refl
    cn-ap {cx} x ≟ cn-ap {cy} y = map′ f g (x =AP= y)
      where
      f : (cx , x) ≡ (cy , y) → cn-ap x ≡ cn-ap y
      f refl = refl
      g : cn-ap x ≡ cn-ap y → (cx , x) ≡ (cy , y)
      g refl = refl
    rcn cx vx ≟ rcn cy vy = map′ f g (vx =VP= vy)
      where
      f : (cx , vx) ≡ (cy , vy) → rcn cx vx ≡ rcn cy vy
      f refl = refl
      g : rcn cx vx ≡ rcn cy vy → (cx , vx) ≡ (cy , vy)
      g refl = refl
    use-cn _ ≟ cn-ap _  = not (λ ())
    use-cn _ ≟ rcn _ _  = not (λ ())
    cn-ap _  ≟ use-cn _ = not (λ ())
    cn-ap _  ≟ rcn _ _  = not (λ ())
    rcn _ _  ≟ use-cn _ = not (λ ())
    rcn _ _  ≟ cn-ap _  = not (λ ())

    _=PN=_ : {cx cy : sCN} (x : sPN cx) (y : sPN cy) → Dec ((cx , x) ≡ (cy , y))
    _=PN=_ {cx} {cy} (use-pn x) (use-pn y) = map′ f g (eqPN x y)
      where
      f : x ≡ y → (cx , use-pn x) ≡ (cy , use-pn y)
      f refl = refl
      g : (cx , use-pn x) ≡ (cy , use-pn y) → x ≡ y
      g refl = refl

    _=DET=_ : (x y : sDET) → Dec (x ≡ y)
    some =DET= some = yes refl
    some =DET= every = not (λ ())
    some =DET= no = not (λ ())
    some =DET= the = not (λ ())
    some =DET= ∅ = not (λ ())
    every =DET= some = not (λ ())
    every =DET= every = yes refl
    every =DET= no = not (λ ())
    every =DET= the = not (λ ())
    every =DET= ∅ = not (λ ())
    no =DET= some = not (λ ())
    ...
    _=NP=_ : {cx cy : sCN} (x : sNP cx) (y : sNP cy) → Dec ((cx , x) ≡ (cy , y))
    _=NP=_ {cx} {cy} (np-pn x) (np-pn y) = map′ f g (x =PN= y )
      where
      f : (cx , x) ≡ (cy , y) → (cx , np-pn x) ≡ (cy , np-pn y)
      f refl = refl
      g : (cx , np-pn x) ≡ (cy , np-pn y) → (cx , x) ≡ (cy , y)
      g refl = refl
    _=NP=_ (np-det dx cx) (np-det dy cy) with dx =DET= dy | cx ≟ cy
    ...
    ... | not dr | _      = not (λ z → dr (f z))
      ...
      f refl = refl
    _=NP=_ (np-pn _) (np-det _ _) = not (λ ())
    _=NP=_ (np-det _ _) (np-pn _) = not (λ ())

    _=Adj=_ : {cx cy : sCN} (x : sAdj cx) (y : sAdj cy) → Dec ((cx , x) ≡ (cy , y))
    _=Adj=_ {cx} {cy} (use-adj x) (use-adj y) = map′ f g (eqAdj x y)
      where
    ...
    _=VI=_ : {cx cy : sCN} (x : sVI cx) (y : sVI cy) → Dec ((cx , x) ≡ (cy , y))
    _=VI=_ {cx} {cy} (use-vi x) (use-vi y) = map′ f g (eqVI x y)
      where
    ...
    _=VT=_ : {cx1 cx2 cy1 cy2 : sCN} (x : sVT cx1 cx2) (y : sVT cy1 cy2)
          → Dec ((cx1 , cx2 , x) ≡ (cy1 , cy2 , y))
    _=VT=_ {cx1} {cx2} {cy1} {cy2} (use-vt x) (use-vt y) = map′ f g (eqVT x y)
      where
      f : x ≡ y → (cx1 , cx2 , use-vt x) ≡ (cy1 , cy2 , use-vt y)
      f refl = refl
      g : (cx1 , cx2 , use-vt x) ≡ (cy1 , cy2 , use-vt y) → x ≡ y
      g refl = refl

    _=VP=_ : {cx cy : sCN} (x : sVP cx) (y : sVP cy) → Dec ((cx , x) ≡ (cy , y))
    _=VP=_ {cx} {cy} (vp-vi vx) (vp-vi vy) = map′ f g (vx =VI= vy)
      where
      ...
      g refl = refl
      -- можно ли упростить?
    _=VP=_ {cx} {cy} (vp-vt {cx1} vtx nx) (vp-vt {cy1} vty ny) with cx ≟ cy | vtx =VT= vty | nx =NP= ny
    ...
    ... | yes cr | yes vtr | yes nr = map′ (f nr) (g nr) (vtx =VT= vty)
      ...
      g refl refl = refl
    -- _=VP=_ {cx} {cy} (vp-vt2 {cx2} vtx nx) (vp-vt2 {cy2} vty ny) with cx ≟ cy | vtx =VT= vty | nx =NP= ny
    -- ... | not ¬p | _      | _      = not \ z → ¬p (f z)
    --   where
    ...
    --   g refl refl = refl
    _=VP=_ (vp-vi _) (vp-vt _ _) = not (λ ())
    -- _=VP=_ (vp-vi _) (vp-vt2 _ _) = not (λ ())
    _=VP=_ (vp-vt _ _) (vp-vi _) = not (λ ())
    -- _=VP=_ (vp-vt2 _ _) (vp-vi _) = not (λ ())
    -- _=VP=_ (vp-vt1 _ _) (vp-vt2 _ _) = not (λ ())
    -- _=VP=_ (vp-vt2 _ _) (vp-vt1 _ _) = not (λ ())
  ...
  mutual
    ...
    ⟦ss s-nvn np1 vt np2 ⟧ = ⟦snp np1 ⟧ (λ x → (⟦snp np2 ⟧ λ y → ⟦svt vt ⟧ x y))   -- = ⟦snp np1 ⟧ ⟦svp vp-vt vt np2 ⟧
  ...
  mutual
    ...
    ...             | just cn' | just vp' with cn' ≟ (proj₁ vp')
...
eqCN Human Human = yes refl
eqCN Dog Dog = yes refl
eqCN Human Dog = not (λ ())
eqCN Dog Human = not (λ ())

eqPN : (x y : namePN) → Dec (x ≡ y)
eqPN Mary Mary = yes refl
eqPN Mary Alex = not (λ ())
eqPN Mary Polkan = not (λ ())
eqPN Alex Mary = not (λ ())
eqPN Alex Alex = yes refl
eqPN Alex Polkan = not (λ ())
eqPN Polkan Mary = not (λ ())
eqPN Polkan Alex = not (λ ())
eqPN Polkan Polkan = yes refl
...
-- a human runs
s4 : S
s4 = s-nv (np-det some (use-cn Human)) (vp-vi (use-vi runs))
...
pp = Hₚ , *Mary-runs
  where
  Hₚ : Pointed *Human -- ⟦scn (use-cn Human) ⟧
  Hₚ = record { theₚ = *Mary }
...
s12 : S
s12 = s-nv (np-pn (use-pn Mary)) (vp-vt (use-vt love) (np-det some (use-cn Human)))
...
-- Every human loves some human (different)

s13 : S
s13 = s-nv (np-det every (use-cn Human)) (vp-vt (use-vt love) (np-det some (use-cn Human)))
...
s17 : S
s17 = s-nvn (np-det every (use-cn Human)) (use-vt love) (np-det some (use-cn Human))
