...
mutual
  ...
  data _<:_ : CN → CN → Set where
    ...
    instance ctr  : ∀ {cn cn1 cn2} → cn <: cn1 → cn1 <: cn2 → cn <: cn2
...
mutual
  ...
  ⟦coe ctr c12 c23 ⟧ = ⟦coe c23 ⟧ ⟪∘⟫ ⟦coe c12 ⟧  -- coerce (getfunc ⟦coe c23 ⟧ ∘ getfunc ⟦coe c12 ⟧)
  ...
  ⟦np_⟧ : {cn cn1 : CN} → NP cn → {{cc : ⟦cn cn ⟧ ⟦<:⟧ ⟦cn cn1 ⟧}} → (⟦cn cn1 ⟧ → Set) → Set
  ...
  {-# TERMINATING #-}
  ⟦vp_⟧ : {cn0 cn01 : CN} → VP cn0 → {{cc : ⟦cn cn01 ⟧ ⟦<:⟧ ⟦cn cn0 ⟧}} → ⟦cn cn01 ⟧ → Set
...
s9 = s-nv a-human-that-runs {{ctr crcn cha}} (vp-vi runs)   -- работает! 
...
s13 = s-nv (np-pn Mary) (vp-vt love {{ctr crcn cha}} a-human-that-runs)
