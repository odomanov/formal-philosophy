...
module Montague0 (E : Set) where         -- e = объекты / тип объектов
...
-- data _/_ {a b : Level} : Set a → Set b → Set (a ⊔ b) where
--   fn : ∀ x y → x / y

data Expr : Level → Set where
  e t : Expr lzero
  _/_ : ∀ {a b} → Expr a → Expr b → Expr (a ⊔ b)
