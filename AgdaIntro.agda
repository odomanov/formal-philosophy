...
Bool-induction : (P : Bool → Set)
               → P false
               → P true
               --------------------
               → (b : Bool) → P b
Bool-induction P x y false = x
Bool-induction P x y true  = y
...
ℕ-ind : (P : ℕ → Set)
      ...
      → (∀ m → P m)
ℕ-ind _ z _  zero   = z
ℕ-ind P z f (suc n) = f (ℕ-ind P z f n)
...
-- Если мы хотим иметь список не только для слов, а для любых типов, то
-- можем определить его как тип с параметром:
...
≡-induction : ∀ {a} {A : Set a} {P : {x y : A} → x ≡ y → Set a}
            → (∀ (x : A) → P (refl {x = x}))
            -------------------------------
            → (x y : A) (p : x ≡ y) → P p
≡-induction r x .x refl = r x
...
-- Таким образом, поля являются функциями, аргументом которых авляется
